...
type ArtworkNode = NonNullable<NonNullable<NonNullable<ArtworkConnectionQuery['artworkConnection']['edges']>[number]>['node']>;
type TagItem = NonNullable<NonNullable<NonNullable<ArtworkNode['tags']>[number]>['tag']>;
...
function ArtworksContent({ query, data, variables }: Props) {
  ...
  const artworks: ArtworkNode[] = (tinaData.artworkConnection.edges ?? []).map((e) => e?.node).filter((n): n is ArtworkNode => n != null);

  const artworkSlug = (a: ArtworkNode) => (a.slug ?? a._sys.filename).toLowerCase();

  const selectedIndex = selectedSlug ? artworks.findIndex((a) => artworkSlug(a) === selectedSlug) : -1;

  const selectedArtwork = selectedIndex >= 0 ? artworks[selectedIndex] : null;

  const goTo = (slug: string) => router.push(`/artworks?artwork=${encodeURIComponent(slug)}`);
  const close = () => router.push('/artworks');
  const prev = () => selectedIndex > 0 && goTo(artworkSlug(artworks[selectedIndex - 1]));
  const next = () => selectedIndex < artworks.length - 1 && goTo(artworkSlug(artworks[selectedIndex + 1]));
  ...
  return (
    <>
      <div className='mx-auto w-full max-w-7xl px-8 py-12 sm:px-10 md:px-[58px]'>
        <p className='mb-2 text-sm uppercase tracking-[0.24em] text-black/45'>Collection</p>
        <h1 className='mb-10 font-space-grotesk text-5xl tracking-[-0.06em] text-black sm:text-6xl'>Artworks</h1>
        {artworks.length === 0 ? (
          <p className='text-black/45'>No artworks yet.</p>
        ) : (
          <div className='grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3'>
            {artworks.map((artwork) => (
              <ArtworkCard key={artwork.id} artwork={artwork} onClick={() => goTo(artworkSlug(artwork))} />
            ))}
          </div>
        )}
      </div>
...
function ArtworkCard({
  ...
  return (
    <button onClick={onClick} className='group text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black'>
      ...
      <h2 className='font-space-grotesk text-xl tracking-[-0.04em] text-black'>{artwork.title}</h2>
      {tags.length > 0 && (
        <div className='mt-2 flex flex-wrap gap-2'>
          {tags.map((tag) => (
            <span key={tag.id} style={{ color: tag.color ?? 'currentColor' }} className='font-space-grotesk text-[10px] uppercase tracking-widest'>
              {tag.title}
            </span>
          ))}
        </div>
