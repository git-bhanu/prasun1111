...
import { ArtworkBentoGrid, ArtworkDetailActions, ArtworkInfoCard } from '@/components/artwork';
...
function ArtworksContent({ query, data, variables, quoteBreaks }: Props) {
  ...
  const [selectedSlug, setSelectedSlug] = useState<string | null>(() => searchParams?.get('artwork') ?? null);
  ...
  const displayIndex = displayArtwork ? artworks.findIndex((a) => artworkSlug(a) === artworkSlug(displayArtwork)) : -1;
