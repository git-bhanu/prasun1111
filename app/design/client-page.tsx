...
import type {
  DesignConnectionQuery,
  DesignConnectionQueryVariables,
...
type DesignNode = NonNullable<
  NonNullable<
    NonNullable<DesignConnectionQuery["designConnection"]["edges"]>[number]
  >["node"]
>;
...
function DesignContent({ query, data, variables }: Props) {
  ...
  const designSlug = (d: DesignNode) =>
    (d.slug ?? d._sys.filename).toLowerCase();

  const selectedIndex = selectedSlug
    ? designs.findIndex((d) => designSlug(d) === selectedSlug)
    : -1;
  ...
  const goTo = (slug: string) => {
    ...
    window.history.pushState(
      {},
      "",
      `/design?design=${encodeURIComponent(slug)}`,
    );
  };
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {otherDesigns.map((d) => (
              <DesignListCard
                key={d.id}
                design={d}
                onClick={() => onItemClick(designSlug(d))}
