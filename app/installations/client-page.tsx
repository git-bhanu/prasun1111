...
function InstallationsContent({ query, data, variables, quoteBreaks }: Props) {
  ...
  return (
    <>
      ...
      >
        {displayInstallation && (
          <DetailPanel installation={displayInstallation} scrollToTop={() => detailScrollRef.current?.scrollTo({ top: 0, behavior: 'smooth' })} />
        )}
      </div>
    </>
  );
    ...
    <>
      ...
      <div className='md:hidden'>
        ...
        <div className='px-4 pt-6 pb-8'>
          ...
          <h1 className='mt-6 font-space-grotesk text-[22px] font-bold leading-[1.2] uppercase text-black' data-tina-field={tinaField(installation, 'title')}>
            {installation.title}
          </h1>
      ...
      <CommentsSection pageSlug={`installations/${(installation.slug ?? installation._sys.filename).toLowerCase()}`} onBackToTop={scrollToTop} />
    </>
  );
}
