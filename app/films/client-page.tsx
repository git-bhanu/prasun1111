...
function FilmDetail({ film }: { film: Film }) {
  const awards = (film.awards ?? []).filter((a): a is AwardItem => a != null && Boolean(a.image));

  const hasHeaderVideo = Boolean(film.heroImage || film.heroVideo || film.youtubeUrl);
        ...
        <div className='px-4 pt-6'>
          {awards.length > 0 && (
            <div className='mb-5 grid grid-cols-3 gap-3'>
              {awards.map((award, i) => (
                <BlurUpImage key={i} src={award.image!} alt={award.alt ?? ''} height={60} width={150} className='w-full object-contain' />
              ))}
            </div>
          )}
          <h1 className='font-space-grotesk text-[28px] font-bold leading-[1.05] text-white' data-tina-field={tinaField(film as any, 'title')}>
            {film.title && <TinaMarkdown content={film.title} components={inlineComponents} />}
          </h1>
          {film.tagline && (
            <p className='mt-3 font-sedan text-[20px] md:text-[24px] text-white mb-4' data-tina-field={tinaField(film as any, 'tagline')}>
              <TinaMarkdown content={film.tagline ?? undefined} components={inlineComponents} />
            </p>
          )}
        </div>

        {film.synopsis && (
          <div className='px-4 pt-3 pb-4 font-sedan text-[16px] md:text-[24px] leading-tight text-white' data-tina-field={tinaField(film as any, 'synopsis')}>
            <TinaMarkdown content={film.synopsis ?? undefined} components={synopsisComponents} />
          </div>
        )}
      ...
      <div className='hidden bg-black px-12 pt-14 pb-16 md:block'>
        {awards.length > 0 && (
          <div className='mb-8 flex flex-wrap gap-4'>
            {awards.map((award, i) => (
              <BlurUpImage key={i} src={award.image!} alt={award.alt ?? ''} height={60} width={150} className='object-contain' />
            ))}
          </div>
        )}
        <h1 className='font-space-grotesk text-[48px] font-bold text-white' data-tina-field={tinaField(film as any, 'title')}>
          {film.title && <TinaMarkdown content={film.title} components={inlineComponents} />}
        </h1>

        {film.tagline && (
          <p className='mt-5 font-sedan text-[14px] md:text-[24px] leading-snug text-white' data-tina-field={tinaField(film as any, 'tagline')}>
            <TinaMarkdown content={film.tagline ?? undefined} components={inlineComponents} />
          </p>
        )}
        ...
        <div className='mt-8 flex items-start justify-between gap-12'>
          <div className='max-w-[600px] border-t border-white/20 pt-8 pr-10'>
            {film.synopsis && (
              <div className='font-sedan text-[14px] md:text-[24px] leading-snug text-white' data-tina-field={tinaField(film as any, 'synopsis')}>
                <TinaMarkdown content={film.synopsis ?? undefined} components={synopsisComponents} />
              </div>
            )}
    ...
    <div className='mt-10 flex w-full flex-col gap-4'>
      <div className='relative aspect-[16/9] w-full overflow-hidden rounded-2xl bg-black'>
        {playing && videoId ? (
          ...
          <CloudinaryPlayer src={film.heroVideo!} poster={film.heroImage ?? undefined} />
        ) : (
          film.heroImage && <BlurUpImage src={film.heroImage} alt='Film' fill className='object-cover' priority />
        )}

        {/* Desktop: watch button overlay top-left */}
        <div className='absolute left-5 top-5 z-10 hidden md:block'>{watchButton()}</div>
      </div>
  ...
  return (
    <div className='border-t pb- border-white/[0.15] mx-4'>
      <div className='grid grid-cols-[1fr_1px_1fr] mt-4'>
        <div>
          {left.map((f, i) => (
            <div key={f.label} className={cn('py-4 pr-4', i < left.length - 1 && 'border-b border-white/[0.15] mr-4')}>
              <p className='font-space-grotesk text-[10px] md:text-[12px] uppercase text-white/50'>{f.label}</p>
              <p className='mt-3 font-space-grotesk text-[14px] md:text-xl uppercase text-white'>{f.value || '—'}</p>
            </div>
          ))}
        </div>
