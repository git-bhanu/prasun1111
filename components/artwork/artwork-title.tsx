...
export function ArtworkTitle({ title, href, arrow = 'up', showArrow, as: Tag = 'h2', className, dataTinaField }: ArtworkTitleProps) {
  ...
  const content = (
    <span>
      {title}
      {shouldShowArrow ? <ArrowIcon className='ml-1 inline h-2.5 w-2.5 translate-y-[-0.30em] fill-current' aria-hidden='true' /> : null}
    </span>
  );

  if (href) {
    return (
      <Link href={href} data-tina-field={dataTinaField} className={titleClassName}>
        {content}
      </Link>
    );
  }
