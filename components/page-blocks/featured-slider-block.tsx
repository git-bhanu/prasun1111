...
type FeaturedSliderSlide = NonNullable<
  NonNullable<PageBlocksFeaturedSlider["slides"]>[number]
>;
type FeaturedSliderSlideField = Exclude<
  keyof FeaturedSliderSlide,
  "__typename"
>;
      ...
      >
        {media.isVideo && media.src ? (
          ...
          <Image
            ...
            sizes={
              variant === "desktop"
                ? "(min-width: 768px) 90vw, 0vw"
                : "(max-width: 767px) 100vw, 0vw"
            }
  ...
  const renderControl = (variant: "previous" | "next") => {
    ...
    return (
      <IconCircleButton
        ...
        aria-label={
          isPrevious
            ? "Show previous featured slide"
            : "Show next featured slide"
        }
      ...
      >
        <Icon
          ...
          color="currentColor"
          className={cn(isPrevious && "rotate-180")}
          aria-hidden="true"
        />
      </IconCircleButton>
    );
    ...
    <section ref={sectionRef} className="bg-white px-4 py-10 md:px-12 md:py-20">
      <div className="mx-auto max-w-[1780px]">
        <div className="hidden md:block">
          <div
            className="relative aspect-[2.75/1] overflow-hidden bg-neutral-50"
            data-tina-field={tinaField(activeSlide, desktopMedia.field)}
          >
        ...
        <div className="mx-auto mt-6 grid w-full max-w-[760px] grid-cols-[4rem_minmax(0,1fr)_4rem] items-start gap-4 md:mt-5 md:grid-cols-[5rem_minmax(0,1fr)_5rem]">
          <div className="flex justify-end pt-0 md:pt-9">
...
function getSlideMedia(
  slide: FeaturedSliderSlide,
  variant: MediaVariant,
): SlideMedia {
  const isMobile = variant === "mobile";
  const assetType = isMobile
    ? slide.mobileAssetType || slide.desktopAssetType
    : slide.desktopAssetType;
  const videoUrl = isMobile
    ? slide.mobileVideoUrl || slide.desktopVideoUrl
    : slide.desktopVideoUrl;
  const image = isMobile
    ? slide.mobileImage ||
      slide.desktopImage ||
      slide.mobileVideoPoster ||
      slide.desktopVideoPoster
  ...
  const imageAlt = isMobile
    ? slide.mobileImageAlt || slide.desktopImageAlt
    : slide.desktopImageAlt;
  ...
  if (isVideo) {
    return {
      ...
      isVideo,
      field:
        isMobile && slide.mobileVideoUrl ? "mobileVideoUrl" : "desktopVideoUrl",
    };
  }
