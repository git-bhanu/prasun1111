...
type InstallationSlide = NonNullable<
  NonNullable<PageBlocksInstallationSlider["slides"]>[number]
>;
type InstallationSlideField = Exclude<keyof InstallationSlide, "__typename">;

const slideEase = [0.22, 1, 0.36, 1] as const;

export function InstallationSliderBlock({
  block,
}: InstallationSliderBlockProps) {
  const slides =
    block.slides?.filter((slide): slide is InstallationSlide =>
      Boolean(slide?.title),
    ) ?? [];
  ...
  const artists =
    activeSlide.artists?.filter((artist): artist is string =>
      Boolean(artist),
    ) ?? [];
  ...
  return (
    <section className="bg-white px-4 py-10 md:px-0 md:py-12">
        ...
        <AnimatePresence initial={false} mode="wait" custom={direction}>
          ...
          >
            ...
            <InstallationDetails
              slide={activeSlide}
              artists={artists}
              className="mt-7 border-t border-black/25 pt-6"
            />
      ...
      <div
        className="relative isolate hidden min-h-[650px] overflow-hidden bg-black md:block"
        data-tina-field={tinaField(activeSlide, mediaField)}
      >
        {renderMedia()}
            ...
            <AnimatePresence initial={false} mode="wait" custom={direction}>
              <motion.div
                ...
                initial={
                  shouldReduceMotion
                    ? { opacity: 1 }
                    : { opacity: 0, y: 12, x: direction * 8 }
                }
                animate={{ opacity: 1, y: 0, x: 0 }}
                exit={
                  shouldReduceMotion
                    ? { opacity: 0 }
                    : { opacity: 0, y: 10, x: direction * -8 }
                }
                transition={contentTransition}
  ...
  return (
    <div
      className={cn(
        ...
        variant === "desktop" && "max-w-[520px] gap-x-7",
        className,
      )}
    >
      ...
      <div className="flex flex-col justify-between gap-10">
        {artists.length ? (
          <div data-tina-field={tinaField(slide, "artists")}>
            ...
            <p
              className={cn(
                "mt-3 font-space-grotesk text-[14px] leading-[1.35] uppercase",
                variant === "desktop" && "text-[20px] text-white",
              )}
            >
  ...
  return (
    <div
      className={cn(
        ...
        variant === "desktop" && withBorder && "border-white/[0.18]",
      )}
    ...
    >
      ...
      <p
        className={cn(
          "mt-1 font-space-grotesk text-[14px]",
          variant === "desktop" && "text-[20px] text-white",
        )}
      >
        {value}
...
function DetailLabel({
  ...
  return (
    <p
      className={cn(
        "font-space-grotesk text-[10px] leading-none text-neutral-500 uppercase",
        variant === "desktop" && "text-sm text-white/50",
      )}
    >
      {children}
    </p>
