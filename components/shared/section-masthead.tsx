  ...
  return (
    <div
      className={cn(
        ...
        size === 'sm' && colorTokens[mobile][borderStrength],
        size === 'sm' && mobileColor && colorTokens[color].mdBorder,
        className
      )}
    >
      {index !== undefined ? (
        <span className={cn('font-space-grotesk font-medium leading-none text-brand-orange -mt-2 -mr-1', sizeClasses[size].index, indexClassName)}>
          {formatIndex(index)}
        </span>
      ) : null}
      <span
